# ----------- #
# Description:
# This script performs the ANOVA test on the A8 dataset.
# the general design is i:h:p
...
class Design8(Design):
    ...
    def get_unique_levels(self):
        ...
        # 'h' is nested within 'p', we must consider the number of unique values for 'h' within each level of 'p'
        # 'i' is nested within 'h', we must consider the number of unique values for 'i' within each level of 'h'
        ...
        # Check that the number of unique values are the same for each level
        if len(n_h.unique()) > 1:
    ...
    def _calculate_degrees_of_freedom(self):
        ...
        self.deg_freedom = {
            ...
            f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}': self.levels[self.corollary_df['p']] * self.levels[self.corollary_df['i']] * (self.levels[self.corollary_df['h']] - 1)
        }
    ...
    def _calculate_T_values(self):
        ...
        self.T = {
            ...
            f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}': np.sum(self.data.groupby([self.corollary_df['p'], self.corollary_df['i'], self.corollary_df['h']])['Response'].mean() ** 2)
        }
    ...
    def _calculate_sums_of_squares(self):
        ...
        self.SS = {
            ...
            f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}': self.T[f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}'] - self.T[f'{self.corollary_df["h"]}:{self.corollary_df["p"]}']
        }
    ...
    def _calculate_mean_squares(self):
        ...
        self.MS = {
            ...
            f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}': self.SS[f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}'] / self.deg_freedom[f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}']
        }
    
    def _calculate_variance(self):
        # ---------------- #
        # Calculate the variances Appendix B.8 in Brennan (2001)
        # a     sigma^2(a)
        ...
        # Get the variances
        self.variances = {
            self.corollary_df['p']: (self.MS[self.corollary_df['p']] - self.MS[f'{self.corollary_df["h"]}:{self.corollary_df["p"]}']) / (self.levels[self.corollary_df['h']] * self.levels[self.corollary_df['i']]),
            f'{self.corollary_df["h"]}:{self.corollary_df["p"]}': (self.MS[f'{self.corollary_df["h"]}:{self.corollary_df["p"]}'] - self.MS[f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}']) / self.levels[self.corollary_df['i']],
            f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}': self.MS[f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}']
        }
    ...
    def calculate_anova(self):
        ...
        # h:p   n_p(n_h-1)          n_i*sum(X_h:p^2)            T(h:p) - T(p)
        # ih:p  n_p*n_h(n_i-1)      sum(X_i:h:p^2)              T(i:h:p) - T(h:p)
        # ---------------- #
    ...
    def _calculate_g_coeffs(self):
        ...
        # p     sigma^2(p)  sigma^2(h:p)/n_h + sigma^2(i:h:p)/(n_h*n_i)                         delta(p)
        # ---------------- #
        ...
        delta_dict = {
            self.corollary_df["p"] : self.variances[f'{self.corollary_df["h"]}:{self.corollary_df["p"]}'] / self.levels[self.corollary_df['h']] + self.variances[f'{self.corollary_df["i"]}:{self.corollary_df["h"]}:{self.corollary_df["p"]}'] / (self.levels[self.corollary_df['h']] * self.levels[self.corollary_df['i']]),
        }
